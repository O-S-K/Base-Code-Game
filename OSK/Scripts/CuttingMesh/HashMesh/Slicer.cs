...
public class Slicer : MonoBehaviour
{
    ...
    void Slice(Collision other)
    {
        ...
        var vec1 = coll.bounds.center;
        var vec2 = coll.bounds.center;
        var vec3 = coll.bounds.center;
        ...
        var pl = new Plane(vec1, vec2, vec3);
        var tr = other.transform;
        var m = other.gameObject.GetComponent<MeshFilter>().mesh;
        var triangles = m.triangles;
        var verts = m.vertices;

        var intersections = new List<Vector3>();
        var newTris1 = new List<Triangle>();
        var newTris2 = new List<Triangle>();
        ...
        for (int i = 0; i < triangles.Length; i += 3)
        {
            ...
            var p1 = tr.TransformPoint(verts[v1]);
            var p2 = tr.TransformPoint(verts[v2]);
            var p3 = tr.TransformPoint(verts[v3]);
            var norm = Vector3.Cross(p1 - p2, p1 - p3);
            ...
            if (points.Count > 0)
            {
                ...
                if (pl.GetSide(p1)) points1.Add(p1);
                else points2.Add(p1);

...
namespace CodeTodoList
{
	public class CTL_Settings
	{
		...
		protected CTL_Exclude mExcludedPaths;
		...
		public static CTL_Exclude ExcludedPaths
		{
			get
			{
				if (Instance.mExcludedPaths == null)
					Instance.mExcludedPaths = new CTL_Exclude();
		...
		{
			bool success = ExcludedPaths.AddExludedPath(path);
			if (success && export)
				Instance.Export();
		}
		...
		public static void AddExludedPath(string[] paths, bool export = true)
		{
			...
			{
				if (ExcludedPaths.AddExludedPath(paths[i]))
					success = true;
			}
		...
		{
			bool success = ExcludedPaths.RemoveExludedPath(path);
			if(success)
				Instance.Export();
		}
